...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initializeDemoBooks() {
        try {
            // Migration: supprimer anciens livres sans couverture
            ...
            if (count == 0) {
                List<Book> demoBooks = createDemoBooks();
                bookRepository.saveAll(demoBooks);
                log.info("{} livres de demonstration ajoutes a la bibliotheque", demoBooks.size());
            }
        } catch (Exception e) {
            log.warn("Impossible d'initialiser les livres de demo: {}", e.getMessage());
        }
    }

    private List<Book> createDemoBooks() {
